...
Wald intervals don't have the best statistical properties:  for smaller sample sizes, and for population proportions that are near 0 or near 1, the nominal level of confidence of the intervals can differ significantly from their actual levels.  In the situation of our study the sample size is not very large, so we recommend a more sophisticated method.  (An Agresti-Coul interval would be fine.)

## Sample Size

Our estimate of $p$, the proportion of households in the population where the stove is still in use, is required to have a minimum precision of 0.10.
...
$$\text{precision} = \frac{\text{margin of error}}{\max(\hat{p}, 1 - \hat{p})} = \frac{z^* \sqrt{\frac{\hat{p}(1 - \hat{p})}{n}}\sqrt{\frac{N-n}{N-1}}}{\max(\hat{p}, 1 - \hat{p})}.$$
