...
We have run some simulation studies to get some idea of the effect of the likely skewness of the population and the non-uniformity of the sample size from household to household on the performance of confidence intervals.  In our simulations the parametric 90%-confidence intervals constructed using all of the data actually cover the mean about 89% of the time.  It may be acceptable to use this interval formula and to keep all of the data.

### The Mixed-Effect Approach

Our problem may also be viewed as a very special case of a linear mixed-effects.  Viewing the problem in this way and using the R-package **nlme**, we get the following results:
...
(**Note:** Intervals obtained from mixed-effects models are a bit wider, as the models are trying to estimate more parameters besides $\mu$.)

### Further Thoughts

We might be able to use the data we have to research how to strike the best balance between the number of sampled households and the number of measurements at each household.  (For example, if a researcher could equally well two villages for two days each in not much more time than it takes to visit one village for four days, could this provide more precise estimates?)
