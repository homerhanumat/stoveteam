...
## What Mean are We Trying to Estimate?
...
Our problem may also be viewed as a very special case of a linear mixed-effects model.  Viewing the problem in this way and using the R-package **nlme**, we get the following results:
...
Note:  More on the bootstrap in subsequent drafts.  Note that we get an interval rather similar to the ones obtained above.
...
Using the bootstrap method with `r B` resamples, we find that:
