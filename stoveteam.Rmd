...
In fact, probability theory tells us that if all households were studied for the same number $J$ of days, then the distribution of $\overline{\overline{X}}$ is normal, with mean $\mu$ and standard deviation:

$$\frac{\sqrt{\sigma^2_b +\sigma^2_w\left(1/J_1+\ldots+1/J_n\right)}}{\sqrt{n}}.$$
...
$$T = \frac{\overline{\overline{X}} - \mu}{\sqrt{\frac{SS_b}{Jn(n-1)}}}$$
would have a t-distribution, with degrees of freedom equal to $n-1$.

One then derives, in a way familiar to statisticians, confidence intervals of the form:

$$\left(\overline{\overline{X}} - t^*\sqrt{\frac{SS_b}{Jn(n-1)}}, \overline{\overline{X}} + t^*\sqrt{\frac{SS_b}{Jn(n-1)}}\right),$$

where $t^*$ is the quantile of the t-distribution appropriate for the desired level of confidence.
...
We can take a conservative approach, however.  Suppose that we were to simply throw away the final measurement for any day that had four measurements.  Then the number $J$ above could be set to three, and from the remaining data we could compute exact confidence intervals.  When we keep the fourth measurements, we can expect our estimates of the mean per-capita usage for those sample households to be more accurate than if we had stayed with just three measurement, so the confidence interval computed using $J=3$ would be a little bit too conservative, wider than it really needs to be.
...
Accordingly, we keep all of our data, but make a conservative 90%-confidence interval using $J = 3$.

Working from the data, we find:
...
* The margin of error for the above interval is `r round(res$margins[2], 3)`.  (By comparison, the margin for an interval based on four measurements for *all* households would have been `r round(res$margins[1], 3)`.)


### A Caution

Recall that one of our assumptions was that the underlying population of mean per-capita household usages was normally distributed.  In practice, no population distribution is ever exaclty normal.  we can take some comfort from statistical theory, which says that even if the population is not normal, the actual coverage-proprties of our confidence intervals converge to the advertised coverage-properties as sample size increases to infinity. Nonetheless, when the sample size is small and the population distribution differs significantly from normal (especially if it is skewed in one direction or another) then there may be significant differences between actual and advertised performance.
...
Note:  Probably won't include this, as non-parametric bootstrap intervals are notoriously wide at small sample sizes.
