...
* for $t_{n_d-1}^*$, will use $t_{n-1}^*$ (which is a bit larger, tending to result in an underestimate of $n_d$);
...
Substituting into our formula, we find $n_d \geq$ `r round(n_d, 3)`, so a sample size of at least `r ceiling(n_d)` households is recommended. In practice, though, we should choose a sample size significantly larger than the minimum $n_d$, as the precision of the resulting estmate, depending as it does on the random quantities $SS_b$ and $\bar{\bar{x}}$, is itself subject to chance variation in the sample, and we want to minimize the chance of that precision coming out too large.
